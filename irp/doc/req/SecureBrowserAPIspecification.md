...
1. R05. **Retrieve information on the environment (device)**. The testing web application will invoke this to gather details about the platform on which it is running. This is used to augment any information that was discernible from the user agent.
	...
	The infoObj is a JSON literal that includes a set of key value pairs. The complete list of data returned is vendor specific. At a minimum, the following keys must be supported: `os`, `name`, `version`, `brand`, `model`, as follows:
	
    * `os` represents the OS type (e.g. Windows, macOS, Linux, iOS, Android, etc.)
    * `name` represents the OS release name, if any (e.g. Sierra, Ubuntu).
    * `version` represents the OS version (e.g. 10.1, 10 Pro, etc.)
    * `brand` represents the secure browser branding (e.g. OAKS, CA, SmarterApp, etc.)
