...
Mozilla.SecurityPrivilege = {
	...
	Disabled : 1, // browser supports enablePrivilege but not enabled right
					// now
	Available : 2
// browser supports enablePrivilege and it is enabled
};
...
Mozilla.allowPrivileged = function() {
	...
	try {
		...
		// NOTE: This stuff won't run anymore since we switching to checking for
		// PrivilegeManager
		Mozilla._lastException = ex;

		// If the error we got was not the browser needing permission then this
		// browser is not mozilla
		var errorMessage = (typeof ex == 'string') ? ex : ex.message; // FF2
																		// exceptions
																		// are
																		// just
																		// strings

		// Check if we got a permission denied error which indicates this
		// browser
...
// You can call this without a callback if you want to see if this browser
// supports execute privileges.
// NOTE: If privileges are denied then this will stop requesting them.
// NOTE: The callback function will only get executed if we gain security
// privileges.
Mozilla.execPrivileged = function(callback, browserMinVersion,
		browserMaxVersion) {
	// check if universal connect was denied
	if (Mozilla._universalConnect == Mozilla.UniversalConnect.Denied)
		return false;
	...
	var minBrowser = YAHOO.lang.isNumber(browserMinVersion) ? browserMinVersion
			: -1;
	var maxBrowser = YAHOO.lang.isNumber(browserMaxVersion) ? browserMaxVersion
			: -1;
...
Mozilla.enableComponents = function() {
	...
	if (window.SecureBrowser && typeof SecureBrowser.Components == 'object') {
		...
		try {
			netscape.security.PrivilegeManager
					.enablePrivilege("UniversalXPConnect"); // enable privilege
															// manager
...
Mozilla.setPreference = function(name, value, browserMinVersion,
		browserMaxVersion) {
	var success = false;
...
// if this is true then we know someone turned on caret mode
Mozilla._caretModeEnabled = false;
...
Mozilla.enableCaretMode = function(enable) {
	...
	if (allowed && enable) {
		Mozilla._caretModeEnabled = true;
	} else {
		Mozilla._caretModeEnabled = false;
	}

	return Mozilla._caretModeEnabled;
};

// Are we in caret mode?
Mozilla.inCaretMode = function() {
	// check if caret mode was manually enabled
	if (Mozilla._caretModeEnabled) {
		// when we know caret mode is enabled then always check pref
		Mozilla._caretModeEnabled = Mozilla
				.getPreference('accessibility.browsewithcaret');
	}
...
Mozilla.preference = function(name, value) {
	...
	if (YAHOO.lang.isUndefined(value)) {
		...
		case prefBranch.PREF_STRING:
			rv = prefBranch.getCharPref(name);
			break;
		case prefBranch.PREF_INT:
			rv = prefBranch.getIntPref(name);
			break;
		case prefBranch.PREF_BOOL:
			rv = prefBranch.getBoolPref(name);
			break;
		default:
			return null;
		}
		...
		if (YAHOO.lang.isString(value))
			...
			rv = prefBranch.setIntPref(name, value);
		else if (YAHOO.lang.isBoolean(value))
			rv = prefBranch.setBoolPref(name, value);
		else if (YAHOO.lang.isNull(value))
			rv = prefBranch.deleteBranch(name);
	}
...
Mozilla.resizeWindowTo = function(width, height) {
	console
			.log('Mozilla.resizeWindowTo is obsolete. Please remove calls to it.');
};
