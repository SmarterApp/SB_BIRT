...
(function(Util) {
	...
	var SUPPORT_ONLY_TOUCH = SUPPORT_TOUCH
	...
	// Get the mac os x version (does not work reliably on older versions
	// 10.3-10.4)
	...
	Browser.getFirefoxVersion = function() {
		...
		if (YAHOO.env.ua.gecko > 0) {
			...
			case 2.0:
				return 4.0;
			default:
				return YAHOO.env.ua.gecko; // NOTE: after 4.0 the gecko version
											// matched firefox version
			}
		}
	...
	Browser.isSafari = function() {
		return YAHOO.env.ua.safari > 0 && !Browser.isEdge()
				&& !Browser.isChrome() && !Browser.isMobile();
	};
	...
	// Summary of Browser Support for MathML:
	...
	// Determines if Ctrl/Alt/Meta keys are available for this browser.
	// Currently iOS is the only one we can't trust
	// due to FB146275 as when CapsLock is on the ctrlKey flag on keyboard
	// events is erroneously set to true.
	...
	Browser.resolveUrl = function(url, root) {
		...
		// escape url
		url = url.split('&').join('&amp;').split('<').join('&lt;').split('"')
				.join('&quot;');
	...
	Browser.requestFullScreen = function(elem) {
		...
		if (elem.webkitRequestFullScreen) {
			// apply className fullScreenMode to fix the size of document.body
