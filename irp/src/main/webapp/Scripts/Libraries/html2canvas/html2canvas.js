/*
 * html2canvas 0.4.1 <http://html2canvas.hertzen.com> Copyright (c) 2013 Niklas
 * von Hertzen
 * 
...
(function(window, document, undefined) {
  ...
  _html2canvas.Util.parseBackgroundImage = function(value) {
    ...
    var appendResult = function() {
      ...
      args = []; // for some odd reason, setting .length = 0 didn't work in
                  // safari
  ...
  function parseBackgroundSizePosition(value, element, attribute, index) {
    ...
    if (attribute === 'backgroundSize'
      ...
      value[0] = (value[0].indexOf("%") === -1) ? toPX(element,
          attribute + "X", value[0]) : value[0];
      ...
      value[1] = (value[1].indexOf("%") === -1) ? toPX(element,
          attribute + "Y", value[1]) : value[1];
    }
    return value;
  }
      ...
      if (prop === 'backgroundSize') {
        if (bgposition[0] === 'auto') {
          ...
          if (/contain|cover/.test(bgposition[0])) {
            var resized = _html2canvas.Util.resizeBounds(image.width,
                image.height, bounds.width, bounds.height, bgposition[0]);
            left = resized.width;
  ...
  _html2canvas.Util.Children = function(elem) {
    ...
    try {
      ...
      _html2canvas.Util.log("html2canvas.Util.Children failed with exception: "
          + ex.message);
      children = [];
    }
    return children;
  ...
  _html2canvas.Util.Font = (function() {
    ...
    return function(font, fontSize, doc) {
      ...
      var container = doc.createElement('div'), img = doc.createElement('img'), span = doc
          .createElement('span'), sampleText = 'Hidden Text', baseline, middle, metricsObj;
  ...
  (function() {
     ...
     * (linear-gradient) support TODO: Add old Webkit -webkit-gradient(radial,
     * ...) support TODO: Maybe some RegExp optimizations are possible ;o)
     */
    Generate.parseGradient = function(css, bounds) {
      ...
      if (m1) {
        ...
        case '-webkit-gradient':
          ...
          gradient = {
            type : m1[2] === 'radial' ? 'circle' : m1[2], // TODO: Add radial
                                                          // gradient support
                                                          // for older mozilla
                                                          // definitions
            x0 : 0,
        ...
        case '-o-radial-gradient':
          ...
          if (m2 && m3) {
            ...
            case 'farthest-side':
              if (m2[0] === 'circle') {
                gradient.rx = gradient.ry = Math.max(gradient.cx, gradient.cy,
                    gradient.x1 - gradient.cx, gradient.y1 - gradient.cy);
              } else { // ellipse
            ...
            case 'contain': // is equivalent to closest-side
              if (m2[0] === 'circle') {
                gradient.rx = gradient.ry = Math.min(gradient.cx, gradient.cy,
                    gradient.x1 - gradient.cx, gradient.y1 - gradient.cy);
              } else { // ellipse
      ...
      if (gradient) {
        ...
        case 'ellipse':
          var canvasRadial = document.createElement('canvas'), ctxRadial = canvasRadial
              .getContext('2d'), ri = Math.max(gradient.rx, gradient.ry), di = ri * 2;

          canvasRadial.width = canvasRadial.height = di;

          grad = ctxRadial.createRadialGradient(gradient.rx, gradient.ry, 0,
              gradient.rx, gradient.ry, ri);
          gradient.colorStops.forEach(addScrollStops(grad));
          ...
          ctx.drawImage(canvasRadial, gradient.cx - gradient.rx, gradient.cy
              - gradient.ry, 2 * gradient.rx, 2 * gradient.ry);
          break;
        }
      }
    ...
    Generate.ListRoman = function(number) {
      var romanArray = [ "M", "CM", "D", "CD", "C", "XC", "L", "XL", "X", "IX",
          "V", "IV", "I" ], decimal = [ 1000, 900, 500, 400, 100, 90, 50, 40,
          10, 9, 5, 4, 1 ], roman = "", v, len = romanArray.length;
  ...
  _html2canvas.Parse = function(images, options) {
    ...
    numDraws = 0, doc = element.ownerDocument, Util = _html2canvas.Util, support = Util
        .Support(options, doc), ignoreElementsRegExp = new RegExp("("
        + options.ignoreElements + ")"), body = doc.body, getCSS = Util.getCSS, pseudoHide = "___html2canvas___pseudoelement", hidePseudoElements = doc
        .createElement('style');
    ...
    function documentWidth() {
      return Math.max(Math.max(doc.body.scrollWidth,
          doc.documentElement.scrollWidth), Math.max(doc.body.offsetWidth,
          doc.documentElement.offsetWidth), Math.max(doc.body.clientWidth,
    ...
    function getCSSInt(element, attribute) {
      ...
      return (isNaN(val)) ? 0 : val; // borders in old IE are throwing 'medium'
                                      // for demo.html
    }
    ...
    function setTextVariables(ctx, el, text_decoration, color) {
      var align = false, bold = getCSS(el, "fontWeight"), family = getCSS(el,
          "fontFamily"), size = getCSS(el, "fontSize"), shadows = Util
    ...
    function renderTextDecoration(ctx, text_decoration, bounds, metrics, color) {
      ...
      case "underline":
        ...
        // TODO As some browsers display the line as more than 1px if the
        // font-size is big, need to take that into account both in position and
        // size
        renderRect(ctx, bounds.left, Math.round(bounds.top + metrics.baseline
            + metrics.lineWidth), bounds.width, 1, color);
        break;
      case "overline":
        renderRect(ctx, bounds.left, Math.round(bounds.top), bounds.width, 1,
            color);
        break;
      case "line-through":
    ...
    function textWrapperBounds(oldTextNode, transform) {
      var parent = oldTextNode.parentNode, wrapElement = doc
          .createElement('wrapper'), backupText = oldTextNode.cloneNode(true);
    ...
    function renderText(el, textNode, stack) {
      var ctx = stack.ctx, color = getCSS(el, "color"), textDecoration = getCSS(
          el, "textDecoration"), textAlign = getCSS(el, "textAlign"), metrics, textList, state = {
        node : textNode,
        textOffset : 0
      };
      ...
      if (Util.trimText(textNode.nodeValue).length > 0) {
        ...
        textList = (!options.letterRendering
            ...
            el, "letterSpacing"))) ? textNode.nodeValue.split(/(\b| )/)
    ...
    function listItemText(element, type) {
      ...
      case "decimal-leading-zero":
        text = (currentIndex.toString().length === 1) ? currentIndex = "0"
            + currentIndex.toString() : currentIndex.toString();
        break;
      case "upper-roman":
    ...
    function clipBounds(src, dst) {
      var x = Math.max(src.left, dst.left), y = Math.max(src.top, dst.top), x2 = Math
          .min((src.left + src.width), (dst.left + dst.width)), y2 = Math.min(
          (src.top + src.height), (dst.top + dst.height));
    ...
    function setZ(element, stack, parentStack) {
      ...
      // "auto",
      // elements with an opacity value less than 1. (See the specification for
      // opacity),
      // on mobile WebKit and Chrome 22+, position: fixed always creates a new
      // stacking context, even when z-index is "auto" (See this post)
    ...
    function renderImage(ctx, element, image, bounds, borders) {
      ...
      var paddingLeft = getCSSInt(element, 'paddingLeft'), paddingTop = getCSSInt(
          ...
          'paddingRight'), paddingBottom = getCSSInt(element, 'paddingBottom');
    ...
    function bezierCurve(start, startControl, endControl, end) {
      ...
      return {
        ...
        subdivide : function(t) {
          var ab = lerp(start, startControl, t), bc = lerp(startControl,
              endControl, t), cd = lerp(endControl, end, t), abbc = lerp(ab,
              bc, t), bccd = lerp(bc, cd, t), dest = lerp(abbc, bccd, t);
    ...
    function calculateCurvePoints(bounds, borderRadius, borders) {
      ...
      return {
        ...
        topLeftInner : getCurvePoints(x + borders[3].width, y
            + borders[0].width, Math.max(0, tlh - borders[3].width), Math.max(
            0, tlv - borders[0].width)).topLeft.subdivide(0.5),
        ...
        topRightInner : getCurvePoints(x
            ...
            + borders[0].width, (topWidth > width + borders[3].width) ? 0 : trh
        ...
        bottomRightInner : getCurvePoints(x
            ...
            + Math.min(rightHeight, height + borders[0].width), Math.max(0, brh
            - borders[1].width), Math.max(0, brv - borders[2].width)).bottomRight
            .subdivide(0.5),

        bottomLeftOuter : getCurvePoints(x, y + leftHeight, blh, blv).bottomLeft
            .subdivide(0.5),

        bottomLeftInner : getCurvePoints(x + borders[3].width, y + leftHeight,
            Math.max(0, blh - borders[3].width), Math.max(0, blv
    ...
    function getBorderClip(element, borderPoints, borders, radius, bounds) {
      ...
      case "padding-box":
        parseCorner(borderArgs, radius[0], radius[1],
            borderPoints.topLeftInner, borderPoints.topRightInner, bounds.left
                + borders[3].width, bounds.top + borders[0].width);
        parseCorner(borderArgs, radius[1], radius[2],
            borderPoints.topRightInner, borderPoints.bottomRightInner,
            bounds.left + bounds.width - borders[1].width, bounds.top
                + borders[0].width);
        parseCorner(borderArgs, radius[2], radius[3],
            borderPoints.bottomRightInner, borderPoints.bottomLeftInner,
            bounds.left + bounds.width - borders[1].width, bounds.top
                + bounds.height - borders[2].width);
        parseCorner(borderArgs, radius[3], radius[0],
            borderPoints.bottomLeftInner, borderPoints.topLeftInner,
            bounds.left + borders[3].width, bounds.top + bounds.height
                - borders[2].width);
        break;

      default:
        parseCorner(borderArgs, radius[0], radius[1],
            borderPoints.topLeftOuter, borderPoints.topRightOuter, bounds.left,
            bounds.top);
        parseCorner(borderArgs, radius[1], radius[2],
            borderPoints.topRightOuter, borderPoints.bottomRightOuter,
            bounds.left + bounds.width, bounds.top);
        parseCorner(borderArgs, radius[2], radius[3],
            borderPoints.bottomRightOuter, borderPoints.bottomLeftOuter,
            bounds.left + bounds.width, bounds.top + bounds.height);
        parseCorner(borderArgs, radius[3], radius[0],
            borderPoints.bottomLeftOuter, borderPoints.topLeftOuter,
      ...
      for (borderSide = 0; borderSide < 4; borderSide++) {
        ...
        if (borders[borderSide].width > 0) {
          ...
          case 0:
            ...
            }, borderRadius[0], borderRadius[1], borderPoints.topLeftOuter,
                borderPoints.topLeftInner, borderPoints.topRightOuter,
          ...
          case 1:
            ...
            }, borderRadius[1], borderRadius[2], borderPoints.topRightOuter,
                borderPoints.topRightInner, borderPoints.bottomRightOuter,
          ...
          case 2:
            ...
            }, borderRadius[2], borderRadius[3], borderPoints.bottomRightOuter,
                borderPoints.bottomRightInner, borderPoints.bottomLeftOuter,
          ...
          case 3:
            ...
            borderArgs = drawSide({
              ...
              c4 : [ bx + bw, by + bh ]
            }, borderRadius[3], borderRadius[0], borderPoints.bottomLeftOuter,
                borderPoints.bottomLeftInner, borderPoints.topLeftOuter,
    ...
    function createShape(ctx, args) {
      ...
      args.forEach(function(border, index) {
        shape[(index === 0) ? "moveTo" : border[0] + "To"].apply(null, border
            .slice(1));
      });
      return shape;
    }
    ...
    function renderFormValue(el, bounds, stack) {
      ...
      var valueWrap = doc.createElement('valuewrap'), cssPropertyArray = [
          'lineHeight', 'textAlign', 'fontFamily', 'color', 'fontSize',
          'paddingLeft', 'paddingTop', 'width', 'height', 'border',
          'borderLeftWidth', 'borderTopWidth' ], textValue, textNode;
      ...
      cssPropertyArray.forEach(function(property) {
        try {
          ...
          Util.log("html2canvas: Parse: Exception caught in renderFormValue: "
              + e.message);
        }
      });
      ...
      Object.keys(elStyle).filter(indexedProperty).forEach(
          function(prop) {
            ...
            try {
              ...
              Util.log([ 'Tried to assign readonly property ', prop, 'Error:',
                  e ]);
            }
          });
    ...
    function injectPseudoElements(el, stack) {
      var before = getPseudoElement(el, ':before'), after = getPseudoElement(
          el, ':after');
      if (!before && !after) {
        return;
      }
    ...
    function renderBackgroundRepeating(el, bounds, ctx, image, imageIndex) {
      var backgroundSize = Util.BackgroundSize(el, bounds, image, imageIndex), backgroundPosition = Util
          .BackgroundPosition(el, bounds, image, imageIndex, backgroundSize), backgroundRepeat = getCSS(
    ...
    function createStack(element, parentStack, bounds, transform) {
      var ctx = h2cRenderContext((!parentStack) ? documentWidth()
          : bounds.width, (!parentStack) ? documentHeight() : bounds.height), stack = {
        ctx : ctx,
      ...
      // TODO correct overflow for absolute content residing under a static
      // position
    ...
    function renderElement(element, parentStack, pseudoElement,
        ignoreBackground) {
      var transform = getTransform(element, parentStack), bounds = getBounds(
          element, transform), image, stack = createStack(element, parentStack,
          bounds, transform), borders = stack.borders, ctx = stack.ctx, backgroundBounds = getBackgroundBounds(
          borders, bounds, stack.clip), borderData = parseBorders(element,
          bounds, borders), backgroundColor = (ignoreElementsRegExp
      ...
      case "INPUT":
        ...
        // todo add support for placeholder attribute for browsers which support
        // it
    ...
    function init() {
      var background = getCSS(document.documentElement, "backgroundColor"), transparentBackground = (Util
          .isTransparent(background) && element === document.body), stack = renderElement(
          element, null, false, transparentBackground);
  ...
  _html2canvas.Preload = function(options) {
    ...
    var images = {
      ...
      numTotal : 0,
      cleanupDone : false
    }, pageOrigin, Util = _html2canvas.Util, methods, i, count = 0, element = options.elements[0]
        || document.body, doc = element.ownerDocument, domImages = element
        .getElementsByTagName('img'), // Fetch images of the present element
                                      // only
    ...
    function isSameOrigin(url) {
      link.href = url;
      link.href = link.href; // YES, BELIEVE IT OR NOT, that is required for
                              // IE9 - http://jsfiddle.net/niklasvh/2e48b/
    ...
    function start() {
      ...
      if (!images.firstRun && images.numLoaded >= images.numTotal) {
        Util.log("Finished loading images: # " + images.numTotal + " (failed: "
            + images.numFailed + ")");
    ...
    function proxyGetImage(url, img, imageObj) {
      ...
      window[callback_name] = function(a) {
        ...
        window[callback_name] = undefined; // to work with IE<9 // NOTE: that
                                            // the undefined callback
                                            // property-name still exists on the
    ...
    function loadPseudoElement(element, type) {
      ...
      if (content.substr(0, 3) === 'url') {
        methods
            .loadImage(_html2canvas.Util.parseBackgroundImage(content)[0].args[0]);
      }
    ...
    function getImages(el) {
      ...
      if (elNodeType === 1 || elNodeType === undefined) {
        ...
        try {
          ...
          Util.log("html2canvas: failed to get background-image - Exception: "
              + e.message);
        }
            ...
            if (images.hasOwnProperty(src)) {
              ...
              if (typeof img === "object" && img.callbackname
                ...
                window[img.callbackname] = undefined; // to work with IE<9 //
                                                      ...
                                                      // object (for IE<9)
                try {
                  delete window[img.callbackname]; // for all browser that
                                                    // support this
                } catch (ex) {
                }
                if (img.script && img.script.parentNode) {
                  img.script.setAttribute("src", "about:blank"); // try to
                                                                  // cancel
                                                                  // running
                                                                  // request
      ...
      rootContext = (function buildStackingContext(rootNode) {
        ...
        function insert(context, node, specialParent) {
          ...
          isPositioned = node.zIndex.isPositioned, isFloated = node.zIndex.isFloated, stub = {
            node : node
          }, childrenDest = specialParent; // where children without z-index
        ...
        Object.keys(context).sort().forEach(function(zi) {
          ...
          context[zi].forEach(function(v) {
            if (v.node.zIndex.isPositioned || v.node.zIndex.opacity < 1) {
              ...
              // non-positioned element with opactiy < 1 should be stacked as if
              // it were a positioned element with ‘z-index: 0’ and ‘opacity:
              // 1’.
              positioned.push(v);
  ...
  _html2canvas.Util.Support = function(options, doc) {
    ...
    // Opera doesn't provide valid bounds.height/bottom even though it supports
    // the method.
  ...
  window.html2canvas = function(elements, opts) {
    ...
    var queue, canvas, options = {
      ...
      useCORS : false, // try to load images as CORS (where available), before
                        // falling back to proxy
      allowTaint : false, // whether to allow images to taint the canvas, won't
                          // need proxy if set to true
      ...
      width : null,
      height : null,
      taintTest : true, // do a taint test with all images before applying to
                        // canvas
      renderer : "Canvas"
    };
  ...
  _html2canvas.Renderer.Canvas = function(options) {
    ...
    return function(parsedData, options, document, queue, _html2canvas) {
      ...
      ctx.fillStyle = (Util.isTransparent(zStack.backgroundColor) && options.background !== undefined) ? options.background
