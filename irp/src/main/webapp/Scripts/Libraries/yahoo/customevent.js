 ...
 */
    YAHOO.util.Event = function() {
        ...
        /*
        // element used to detect if browser supports a feature
        var featureEl;

        // check if browser supports dom event
        function hasEvent(eventName) {
            eventName = 'on' + eventName;
            if (!featureEl) {
                featureEl = document.createElement('div');
            }
            return eventName in featureEl;
        }

        // check for native pointer event support (ignores PEP.js)
        var isPointerSupported = hasEvent('pointerdown');
        */

        var isTabletDevice = (YAHOO.env.ua.android > 0 || YAHOO.env.ua.ios > 0);
        ...
        var touchEvents = {
            ...
            move: 'touchmove',
            up: 'touchend',
            // NOTE: these don't actually work, you should rely on touchmove and figure it out yourself
            over: 'touchenter',
            out: 'touchleave',
            enter: 'touchenter',
            leave: 'touchleave'
        ...
        // get the events that we will use in place of the mouse events
        function getSupportedMouseEvents() {
            // NOTE: There is a very specific order to how we return the supports events. Please be careful! 
            if (isTabletDevice && isTouchSupported) { // ios/android
                return touchEvents;
            } else if (window.PointerEvent) { // pointer event (including PEP.js)
                return pointerEvents;
            } else if (isTouchSupported) { // other touch enabled device
                return touchEvents;
            } else { // desktop
                return mouseEvents;
            }
        ...
        return {
            ...
            * @deprecated use YAHOO.util.Event.on and specify "focusin" as the event type.
            ...
            // make a touch event look like a mouse event
            normalizeTouchEvent: function(ev) {
                ...
                if (ev && ev.changedTouches) {
                    ...
                    for (var i = 0, ii = touches.length; i < ii; i++) {
                        // find touch event that matches dom event by checking target
                        if (touches[i].target === ev.target) {
                            // save original event
                            var oldEv = ev;
                            ...
                            ev.preventDefault = function () {
                                return oldEv.preventDefault();
                            };
                            ev.stopPropagation = function () {
                                return oldEv.stopPropagation();
                            };
                            break;
                        }
                    }
             ...
             * @method getEvent
             * @param {Event} ev the event parameter from the handler
            ...
            getEvent: function (ev, boundEl) {
                ...
                if (ev) {
                    ev = this.normalizeTouchEvent(ev);
                } 
