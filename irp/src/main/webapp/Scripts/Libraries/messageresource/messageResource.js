...
(function(global) {
  ...
  /**
   * Module with methods for loading and using message resource property files
   * in javascript.
   * 
  ...
  var messageResource = (function() {
    ...
    DEFAULT_EXTENSION = '.properties', filePath, fileExtension, defaultLocale = 'en_US', currentLocale = defaultLocale, fileNameResolver, ajaxFunction,
    // status flag for checking valid configuration.
    validConfiguration = false, debugMode = false;
     ...
     * 
     * @param {String}
     *          text - Contents of message resource file.
     * @param {String}
     *          module - A valid module name.
     * @param {String}
     *          locale - A valid locale identifier.
     * @private
     */
    function saveFile(text, module, locale) {
      var linesArray, curModuleMap;
    ...
    /**
     * Get a valid locale name. If null is passed, returns default or current
     * configured locale.
     * 
     * @param {String}
    ...
    function isModuleLoaded(module, locale) {
      ...
      if (module && locale) {
        moduleLoaded = (properties && properties[locale] && properties[locale][module]) ? true
            : false;
      }
      return moduleLoaded;
    }
    ...
    function convertUnicodeString(str) {
      var convertedText = str.replace(/\\u[\dA-Fa-f]{4}/g,
          function(unicodeChar) {
            return String.fromCharCode(parseInt(
                unicodeChar.replace(/\\u/g, ''), 16));
          });
     ...
     *          url - Url to load
     * @param {Function}
     *          callback - Callback to be executed after loading.
     * @private
     */
    ...
    return {
       ...
       * @param {String}
       *          [config.filePath] - Path(directory) of message resource files.
       * @param {String}
       *          [config.fileExtension = .properties] - File extension of
       *          message resource files.
       * @param {String}
       *          [config.defaultLocale = en_US] - Default locale.
       * @param {Function}
       *          [config.fileNameResolver = defaultFileNameResolver] - Specify
       *          custom file name resolver.
       * @param {Function}
       *          [config.ajaxFunction = ajaxGet] - Specify custom ajax function
       *          for loading files. The function should accept only 2 arguments :
       *          1. Url/path of the file. 2. Callback with response text as
       *          argument.
       * @param {Boolean}
       *          [config.debugMode = false] - Enable/Disble debug mode.
       * @public
       */
      ...
      /**
       * Set current locale to be used. This configured locale will be used by
       * load and get functions if locale not specified. If current locale is
       * not set config.defaultLocale will be used as current locale, which is
       * en_US by default.
       * 
       * @param {Sring}
       *          locale - Locale identifier like en_US.
      ...
      /**
       * Load a message resource file. The file name is constructed based on the
       * given module name and locale.
       * 
       * File name is constructed with default configuration in different cases
       * as follows :
       * 
       * case 1 : Module name and locale can be empty or null, then the file
       * name will be resolved to _default.properties. case 2 : Module name -
       * HomePage and locale - empty, then file name - HomePage.properties. case
       * 3 : Module name - empty and locale - en_US, then file name -
       * _default_en_US.properties. case 4 : Module name - HomePage and locale -
       * en_US, then file name - HomePage_en_US.properties.
       * 
       * The above file name construction logic can be overriden by configuring
       * custom fileNameResolver functions.
       * 
       * @param {String |
       *          Array} [module = DEFAULT_MODULE_NAME] - Module name or list of
       *          module names.
       * @param {String}
       *          [locale] - Locale identifier like en_US. Configured current
       *          locale will be used if not given.
       * @param {Function}
       *          callback - Callback to be executed after loading message
       *          resource.
       * @public
       */
      load : function(module, callback, locale) {

        var fileLocale, validLocale, validModule, modulesToLoad = [], i;

        if (!validConfiguration) {
          log(
              'Invalid configuration - Invoke init method with proper configuration',
              true);
          return;
        }
        ...
        if (Array.isArray(validModule)) {
          ...
          for (i = 0; i < validModule.length; i++) {
            if (validModule[i] && !isModuleLoaded(validModule[i], validLocale)) {
       ...
       * 
       * @param {String}
       *          key - Message resource property key.
       * @param {String}
       *          [module = DEFAULT_MODULE_NAME] - Module name
       * @param {String}
       *          [locale] - Locale identifier like en_US. Configured current
       *          locale will be used if not given.
       * @param {String}
       *          defaultValue - Default value to return if value not found. If
       *          defaultValue is empty key will be used as defaultValue.
       * @return {String} Message resource property value if exists else
       *         defaultValue passed.
       * @public
       */
      get : function(key, module, locale, defaultValue) {
        var validModule, validLocale, moduleObj, value = defaultValue || key;
