...
function TTSService_Chrome() {
  ...
  this.load = function() {
    ...
    window
        .addEventListener(
            ...
            function(event) {
              ...
              try {
                if (event.data.type
                    && (event.data.type == "CHROME RESPONSE" && event.data.status == "OK")) {
                  switch (event.data.command) {
                  case 'TTS INIT':
                    if (thisObj.status == TTS.Status.Initializing) { // Only if
                      // we are
                      // waiting
                      // to
                      // initialize.
                    ...
                    break;
                  case 'TTS STATUS':
                    // TTS.Config.Debug && console.log("TTS Chrome event
                    // Status.", event.data.type, event);
                    // We get status updates from the background telling us
                    // about speech progress
                    ...
                    break;
                  case 'TTS WORD':
                    // TTS.Config.Debug && console.log("TTS Chrome event WORD.",
                    // event.data.type, event);
                    thisObj.EventManager && thisObj.EventManager.fire({
                      subject : 'TTS WORD',
                      index : event.data.result
                    });
                    break;
                  }
                }
    ...
    nativeInit();
    // Now put this into a timer in case our previous call to nativeInit did not
    // succeed. This can happen
    // as for the chrome packaged app (for the chrome extension, this won't
    // happen),
    // the TDS.AppWindow does not get set until DOMLoad is done and we could get
    // into a case
  ...
  this.subscribe = function(EventManager) {// Subscribe to the word boundary
    // events.
  ...
  this.getStatus = function() {
    ...
    // If we are still initialzing, lets not send anything to the native engine
    ...
    // console.log("TTSChrome.getStatus(), Posting a message to the plugin with:
    // ", params, this.status);
    return this.status;
  };
  ...
  this.play = function(text) {
    ...
    // rate: 1.0 is the default rate, normally around 180 to 220 words per
    // minute. 2.0 is twice as fast, and 0.5 is half as fast
    // pitch: between 0 and 2 inclusive, with 0 being lowest and 2 being
    // highest. 1.0 corresponds to a voice's default pitch.
    // volume: between 0 and 1 inclusive, with 0 being lowest and 1 being
    // highest, with a default of 1.0.
    var param = [ text, {
      rate : (this.rate / 10) * 1,
      voiceName : this.currentVoice,
      lang : this._extractLangCode(this.currentVoice), // This is a hack.
      // Chromebook does not
      // seem to honor
      // voiceName but does
      // honor lang
    ...
    if (Util.Browser.isChromeOS()) {
      if (Util.Browser.getChromeVersion() < 34) {
        this.status = TTS.Status.Playing; // HACK! We really dont need to do
        // this because we are listening for
        // STATUS but on chrome books, we are
        // not getting 'start' event on speech
        // start
      } else {
        // Bug 133270 Chromebook v34 doesn't generate interrupt for first word
        this.EventManager && this.EventManager.fire({
          subject : 'TTS WORD',
          index : 0
        });
      }
    ...
    this.showingProgress = setTimeout(function() {
      TTS.Manager.Events.onShowProgress.fire(Messages.getAlt(
          'TDSTTS.Label.TTSwait', 'Waiting for Text-to-Speech'));
    }, 1000);
    return true;
  };
  ...
  // This is a hack. We need to figure out the lang codes from the chrome voice
  // name
  // This is needed because chromebooks are not honoring the voice name but
  // honor the lang attribute
  // for chrome.tts.speak()
  this._extractLangCode = function(voiceName) {
    if (voiceName && voiceName == 'native')
      return ''; // This is a hack for our extension/app running in desktop
    // chrome!!!
    if (voiceName && voiceName.indexOf("Spanish") >= 0)
      return 'es-ES';
    return 'en-US';
  };
}
