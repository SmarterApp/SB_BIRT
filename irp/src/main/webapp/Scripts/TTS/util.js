...
TTS.Util = (function() {
  ...
  var ttsUtil = {
      ...
      if (elmObj.nodeName == 'SPAN') { // Supports the old way of tagging.
        if (YUD.hasClass(elmObj, 'tts') || YUD.hasClass(elmObj, 'TTS')) {
          if (elmObj.attributes != null
              && elmObj.attributes.getNamedItem("ssml") != null) {
     ...
     * Node.ELEMENT_NODE) { if (YUD.hasClass(domNode, 'languagedivider')) {
     * return domNode; } } } return null; },
     */
    ...
    bubble : function(node, match, results) { // Bubble up the tree looking for
      ...
      if (node && typeof match == 'function') {
        if (match(node)) {
          results.node = node;
        } else if (node.parentNode && node.nodeName != "BODY"
    ...
    cleanText : function(text, addHacks) { // controls speech..
      ...
      if (TTS.Config.shouldSanitizeTextForTTS()) {
        // Remove all non ASCII characters outside the standard ASCII set with
        // the exception of the Spanish characters
        // Spanish character list from
        // http://webdesign.about.com/od/localization/l/blhtmlcodes-sp.htm#codes
        text = text
            .replace(
                /[^\u0000-\u0081\u00C1\u00E1\u00C9\u00E9\u00CD\u00ED\u00D1\u00F1\u00D3\u00F3\u00DA\u00FA\u00DC\u00FC\u00AB\u00BB\u00BF\u00A1\u2047]+/g,
                "");
    ...
    replaceLeadDirectives : function(text) {
      var replaceText = function(expString, target) {
        ...
        // Create regular expression to find TTS prefix and leading replacement
        // target
    ...
    // Bug 114921 Silenced tables cause excessive silences to occur. Resolve by
    // making 2nd - nth silence 1msec duration
    // Note: Replaced silence string must be the same length as the original
    // string, or else TTS highlighting gets out of sync
    shortenSilence : function(text) {
      if (Util.Browser.isWindows()) {
        ...
        for (var i = shortenSilence.length - 1; i > 0; i--) {
          var filteredShortenSilence = i > 0 ? shortenSilence[i - 1].replace(
              /[ ,]/g, '') : null;
          if (filteredShortenSilence
              && filteredShortenSilence.search(filteredSilence) == 0) {
            shortenSilence[i] = shortenSilence[i].replace(silence,
                '<silence msec="1"  ');
          }
        }
        ...
        for (i = shortenSilence.length - 1; i > 0; i--) {
          filteredShortenSilence = i > 0 ? shortenSilence[i - 1].replace(
              /[ ,]/g, '') : null;
          if (filteredShortenSilence
              && filteredShortenSilence.search(filteredSilence) == 0) {
            shortenSilence[i] = shortenSilence[i]
                .replace(silence, '[[slnc 1  ');
          }
        }
        text = shortenSilence.join(']]');
      }
    ...
    /*
     * Remove all the {silence:xxx} directives and replace with the OS specific
     * silence markup For windows - it is <silence="400ms"/> For OS X - it is
     * [[slnc 400]] For Linux - we dont have anything ... We have been using ;
     * as a hack everywhere so I will use that here
     */
