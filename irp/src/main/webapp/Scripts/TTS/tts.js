...
TTS.Manager = {
  Events : {
    ...
    // when the TTS is requested to play, show progress spinner until playing
    // begins
    onShowProgress : new YAHOO.util.CustomEvent('onShowProgress', TTS.Manager,
        false, YAHOO.util.CustomEvent.FLAT),

    // when the TTS has started playing, hide progress spinner
    onHideProgress : new YAHOO.util.CustomEvent('onHideProgress', TTS.Manager,
        false, YAHOO.util.CustomEvent.FLAT),

    // when the TTS service provider is available to the TTS manager for
    // initialization and use
    onServiceLoad : new YAHOO.util.CustomEvent('onServiceLoad', TTS.Manager,
        false, YAHOO.util.CustomEvent.FLAT),

    // when the TTS manager gets successfully initialized (NOTE: if java applet
    // then could take a while)
    onInitSuccess : new YAHOO.util.CustomEvent('onInitSuccess', TTS.Manager,
        false, YAHOO.util.CustomEvent.FLAT),

    // when the TTS manager fails to initialize because there are no service
    // providers or they had an error
    onInitFailure : new YAHOO.util.CustomEvent('onInitFailure', TTS.Manager,
        false, YAHOO.util.CustomEvent.FLAT),

    // when the tts status gets changed
    onStatusChange : new YAHOO.util.CustomEvent('onStatusChange', TTS.Manager,
        false, YAHOO.util.CustomEvent.FLAT),

    // when the volume level is changed (NOTE: this is fired on init)
    onVolumeChange : new YAHOO.util.CustomEvent('onVolumeChange', TTS.Manager,
        false, YAHOO.util.CustomEvent.FLAT),

    // when the pitch level is changed (NOTE: this is fired on init)
    onPitchChange : new YAHOO.util.CustomEvent('onPitchChange', TTS.Manager,
        false, YAHOO.util.CustomEvent.FLAT),

    // when the rate level is changed (NOTE: this is fired on init)
    onRateChange : new YAHOO.util.CustomEvent('onRateChange', TTS.Manager,
        false, YAHOO.util.CustomEvent.FLAT),

    // when a new voice is selected (NOTE: this is fired on init)
    onVoiceChange : new YAHOO.util.CustomEvent('onVoiceChange', TTS.Manager,
        false, YAHOO.util.CustomEvent.FLAT),

    // when a error occurs
    onError : new YAHOO.util.CustomEvent('onError', TTS.Manager, false,
        YAHOO.util.CustomEvent.FLAT),

    // Each time a word boundary is reached by the speech engine.
    onWord : new YAHOO.util.CustomEvent('onWord', TTS.Manager, false,
        YAHOO.util.CustomEvent.FLAT),

    // if interested in logging..
    onLog : new YAHOO.util.CustomEvent('onLog', TTS.Manager, false,
        YAHOO.util.CustomEvent.FLAT)
  },
  ...
  _statusPollingMinActive : 3000, // the min amount of time before going to idle
  // interval even if stop is sooner
  _statusPollingDateActive : null, // the date the status interval last went to
  // active
...
/*
 * we have two different cases 1. we are dealing with a language for which a
 * voice pack is not available on THIS machine. 2. we are dealing with a
 * language for which there are no voice packs at all e.g. Hawaii. in the first
 * case we want to show the menu but disable it. in the second case we do not
 * want to show the menu at all. TTS.Manager already has a isLanguageSupported()
 * method. however, using that we cannot handle the second case. So we will use
 * this alternate method to handle case 2.
 */
...
// This function will load up the proper TTS service for this browser and
// initialize the TTS engine
TTS.Manager.init = function(forceInit) { // set forceInit to true if this is a
  // re-initialization
  ...
  try {
    ...
    // If you are running in SB, use the SB's native TTS capabilities.
...
TTS.Manager.play = function(text, language) {
  ...
  try {
    ...
    // on windows it does not queue, just finishes playing the current one
...
TTS.Manager.stop = function() {
  ...
  var currentStatus = TTS.Manager.getStatus();
  if (currentStatus != TTS.Status.Playing
      && currentStatus != TTS.Status.Unknown
...
TTS.Manager.getVoices = function() {
  ...
  TTS.Config.Debug
      && console.log("getVoice call returned.", voices, TTS.Manager._service);
  return voices;
};
...
/**
 * Attempt to load the user settings. This is required because people load
 * broken voice packs into their OS. They show up as "voices" but playing with
 * them fails.
 */
TTS.Manager.loadUserSettings = function() {
  try {
    ...
    // Set the preferred voices for each language using the value that may have
    // been configured
    // in the TTS configuration page
    var langCodes = TTS.Manager.getKnownLanguageCodes();
    var curVoicePack = null;
    for (var i = 0; i < langCodes.length; i++) {
      var langVoicePref = TTS.Config.User.getVoice(langCodes[i]);
      if (langVoicePref) {
        TTS.Config.Debug
            && console.log("Setting a user preference for the language.",
                langVoicePref);
    ...
    // Setup a default voice with proper volume, pitch, and rate
    if (curVoicePack) {
      var curLang = curVoicePack.Language;
      var v = TTS.Manager.getVoice();
      if (v != curVoicePack.ServiceVoiceName) {
        TTS.Manager.setVoice(curVoicePack.ServiceVoiceName);
      }
...
TTS.Manager.setVolume = function(level) {
  ...
  if (!YAHOO.util.Lang.isNumber(level)) {
    throw new Error('Invalid volume level: value=' + level + ' type='
        + typeof level);
  }
  try {
    // Hack: Chromebooks have a problem where volume 0 is sounding like volume
    // 10 and I've been told volume 0 should not
    // make TTS inaudible.
    // TODO: Update sliders so volume range is 1-10 and, while we're at it, rate
    // is 1-20 (as we do this same basic hack
    // for rate later in this file)
    if (level < 1) {
      level = 1;
    }
...
TTS.Manager.setPitch = function(level) {
  ...
  if (!YAHOO.util.Lang.isNumber(level)) {
    throw new Error('Invalid pitch level: value=' + level + ' type='
        + typeof level);
  }
...
TTS.Manager.setRate = function(level) {
  ...
  if (!YAHOO.util.Lang.isNumber(level)) {
    throw new Error('Invalid rate level: value=' + level + ' type='
        + typeof level);
  }

  try {
    if (level < 1) { // Don't allow a rate of 0 as that basically means don't
      // speak and will cause the student app to go into an
      // endless wait when speaking
      level = 1;
    }
...
TTS.Manager.isAvailable = function() {
  ...
  return ttsStatus == TTS.Status.Stopped || ttsStatus == TTS.Status.Playing
      || ttsStatus == TTS.Status.Paused;
};
...
TTS.Manager._updateStatus = function() {
  ...
  if (this._statusPollingInterval > 0) {
    TTS.Manager._statusPollingTimer = YAHOO.lang.later(
        this._statusPollingInterval, TTS.Manager, TTS.Manager._updateStatus);
  }
};
...
TTS.Manager._changeStatus = function(currentStatus) {
  ...
  // if this is the first time we are playing we need to fix the volume (for
  // issues on OS X?)
  if (currentStatus == TTS.Status.Playing && !TTS.Manager._volumeFix) {
    if (!TTS.Manager._volumeFix) {
      var volume = TTS.Manager._lastVolume >= 0 ? TTS.Manager._lastVolume : 10; // 10
      // is
      // max
      // volume
      TTS.Manager.setVolume(volume);
      var pitch = TTS.Manager._lastPitch >= 0 ? TTS.Manager._lastPitch : 10; // 10
      // is
      // medium
      // pitch
      TTS.Manager.setPitch(pitch);
      var rate = TTS.Manager._lastRate >= 0 ? TTS.Manager._lastRate : 10; // 10
      // is
      // moderate
      // rate
...
/*
 * We will only set the "Available" attribute of the voice packs. All other lets
 * see what voice packs are available with the service.this method is internally
...
TTS.Manager._initializeAvailableVoices = function() {
  ...
  TTS.Config.Debug
      && console.log("TTS.Manager Available Voices: ", availableVoices);
  ...
  /*
   * we will now need to loop through and adjust priorities. we actually do not
   * care about language because priorities are all relative.
   */
  for (var counter1 = 0; counter1 < TTS.Manager._knownVoicePacks.length; ++counter1) {
    var voicePackElement = TTS.Manager._knownVoicePacks[counter1];
...
TTS.Manager.registerVoicePack = function(voicename, priority, language) {
  ...
  TTS.Manager._knownVoicePacks.push(new TTS.VoicePack(voicename, priority,
      language, false, false));
  // add the language to the list of languages for which we know we have voice
  // packs.
...
/*
 * we will try to find the highest priority available voice pack for this
 * language. if nothing works out then we will return default.
 */
...
/*
 * we will try to set the highest priority available voice pack for this
 * language. if nothing works out then we will use default.
 */
TTS.Manager.setVoiceForLanguage = function(language) {
  ...
  if (!selectedVoice || selectedVoice == currentVoicePack) {
    ...
    if (currentVoicePack
        && (selectedVoice.Language == currentVoicePack.Language)) {
...
// go through all voice packs and check if at least one voice is available for
// this language.
...
TTS.Manager.setAsPreferredVoicePack = function(voicePack) {
  if (voicePack.Available) {
    voicePack.Priority = 0; // The lowest Priority voice is always picked for a
    // given language
  }
};
