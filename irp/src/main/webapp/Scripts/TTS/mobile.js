...
function TTSService_MobileSB() {
  ...
  this.voicepacks = [];
  this.currentVoice = '';
  this.pauseEnabled = (Util.Browser.isAndroid()) ? false : true;
  /*
   * pause and resume are not supported on Android Browser
   */
  ...
  this.supportsVolumeControl = function() {
    ...
    /*
     * we only allow volume control for TTS on iOS browser later than version
     * 3.0
     */
  ...
  this.load = function() {
    ...
    this.browserComponent = (new Summit.SecureBrowser.Mobile())
        .getNativeBrowser();
  ...
  this.subscribe = function(EventManager) {
    try {
      ...
      this.browserComponent.listen(
          this.browserComponent.EVENT_TTS_SYNCHRONIZED, document, function(
              result) {

            service = that;
            if ((result) && (result.data)) {
              service.eM.fire({
                index : result.data.location,
