...
(function(SB) {
	...
	Firefox.prototype.checkGlobalObject = function() {
		...
		Util.Validation.setResultItems(1,
				'[SecureBrowser] global object check', 'SecureBrowser', result,
				details);
	...
	Firefox.prototype.checkTTSStopAPI = function() {
		...
		Util.Validation.setResultItems(2,
				'Stop speech (text-to-speech synthesis)', 'runtime.stop()',
				result, details);
	};
	...
	Firefox.prototype.getProcessList = function(allowDups) {
			...
			try {
				...
				if (typeof (this.runtime.services) == "string") {
					...
					if (!Util.String.isNullOrEmpty(serviceString)) {
						// add the parent 'unavailable' for each service to be
						// structually consistent with process list
		...
		processList = (allowDups === true) ? processList : Util.Array
				.unique(processList);
	...
	Firefox.prototype.killProcess = function(processName) {
		...
		if (Util.Browser.getSecureVersion() >= 8 && this._hasRuntime()) {
			try {
				// kill a process, this feature only available on Secure Browser
				// 8 or later versions
				if (processName) {
	...
	Firefox.prototype.startProcess = function(processName) {
		...
		if (Util.Browser.getSecureVersion() >= 8 && this._hasRuntime()) {
			try {
				// start a process, this feature only available on Secure
				// Browser 8 or later versions
				if (processName) {
	...
	Firefox.prototype.fixFocus = function() {
		...
		this._ignoringBreachEvents = setTimeout(stopIgnoringBreachEvents
				.bind(this), 1000);
		...
		try {
			...
			var hiddenWindowRef = w.openDialog(
					"chrome://bmakiosk/content/hiddenWindow.xul", "hidden",
					"alwaysLowered=yes,titlebar=no");
			setTimeout(function() {
	...
	Firefox.prototype.close = function() {
		...
		if (Util.Browser.isMac() && Util.Browser.osxVersionIsAtLeast(10, 8)
				&& (Util.Browser.getSecureVersion() <= 6.2)) {
	...
	Firefox.prototype.getAppStartTime = function() {
		...
		try {
			var startTimePref = Mozilla
					.getPreference("bmakiosk.startup.timestamp");
	...
	// get system volume
	Firefox.prototype.getVolume = function() {
		try {
			if (this._hasRuntime()) {
				if (this.isMuted()) {
					return 0; // muted
	...
	// set system volume
	Firefox.prototype.setVolume = function(percent) {
		try {
			if (this._hasRuntime()) {
	...
	Firefox.prototype.enablePermissiveMode = function(enabled) {
		...
		if (changed && Util.Browser.isMac()) {
			...
			/*
			 * NOTE: On Mac when we turn on permissive mode the bar at the top
			 * appears. When this bar shows up it covers the top of the browser.
			 * If we get the available height then it will take into account the
			 * bar. So we just need to resize the window to have it fit
			 * properly.
			 */
	...
	/*
	 * Register for SB events that indicate that some other app has forced
	 * itself on top of the SB. This is treated as a security breach
	 */
		...
		var observer = {
			...
			observe : function(subject, topic, data) {
				if (topic == "sb-security-breach"
						&& !self._ignoringBreachEvents) {
					Util.log('Security breach detected!');
					var notification = {
						type : 'sb-security-breach',
						message : 'Event from SB'
					};
	...
	/*
	 * Unscribe all listeners that may have hooked into the SB for security
	 * breach events. This is required to make sure that we dont leak memory
	 */
	...
	Firefox.prototype.disableKeyboardLayoutChanges = function() {
		...
		if (Mozilla.getPreference("windows.previouslanguagehotkeyvalue") == null) {
			var languageHotKeyValue = this.readRegistryValue(
					Util.SecureBrowser.HKEY_CURRENT_USER,
					"Keyboard Layout\\toggle", "Language Hotkey");
			if (languageHotKeyValue == null || languageHotKeyValue != '3') {
				...
				this.writeRegistryValue(Util.SecureBrowser.HKEY_CURRENT_USER,
						"Keyboard Layout\\toggle", "Language Hotkey", "string",
						"3");
			}
			...
			if (languageHotKeyValue == null) {
				Mozilla
						.setPreference("windows.previouslanguagehotkeyvalue",
								"");
			} else {
		...
		if (Mozilla.getPreference("windows.previouslayouthotkeyvalue") == null) {
			var layoutHotKeyValue = this.readRegistryValue(
					Util.SecureBrowser.HKEY_CURRENT_USER,
					"Keyboard Layout\\toggle", "Layout Hotkey");
			if (layoutHotKeyValue == null || layoutHotKeyValue != '3') {
				// set the new registry key value
				this.writeRegistryValue(Util.SecureBrowser.HKEY_CURRENT_USER,
						"Keyboard Layout\\toggle", "Layout Hotkey", "string",
						"3");
			}
	...
	Firefox.prototype.restoreKeyboardLayoutSettings = function() {
		...
		if (Mozilla.getPreference('windows.previouslanguagehotkeyvalue') == '') {
			this.removeRegistryValue(Util.SecureBrowser.HKEY_CURRENT_USER,
					"Keyboard Layout\\toggle", "Language Hotkey");
			...
			this
					.writeRegistryValue(
							...
							"string",
							Mozilla
									.getPreference('windows.previouslanguagehotkeyvalue'));
		}
		...
		if (Mozilla.getPreference('windows.previouslayouthotkeyvalue') == '') {
			...
			this.writeRegistryValue(Util.SecureBrowser.HKEY_CURRENT_USER,
					"Keyboard Layout\\toggle", "Layout Hotkey", "string",
					Mozilla.getPreference('windows.previouslayouthotkeyvalue'));
		}
		// remove the value from the local storage
	...
	Firefox.prototype.writeRegistryValue = function(hkey, key, valueName, type,
			value) {
		Mozilla.execPrivileged(function() {
			try {
