...
(function(SB) {
  ...
  Firefox.prototype.fixFocus = function() {
    ...
    this._ignoringBreachEvents = setTimeout(
        stopIgnoringBreachEvents.bind(this), 1000);
  ...
  Firefox.prototype.enablePermissiveMode = function(enabled) {
    ...
    if (changed && Util.Browser.isMac()) {
       ...
       * appears. When this bar shows up it covers the top of the browser. If we
       * get the available height then it will take into account the bar. So we
       * just need to resize the window to have it fit properly.
       */
  ...
  /*
   * Register for SB events that indicate that some other app has forced itself
   * on top of the SB. This is treated as a security breach
   */
  ...
  /*
   * Unscribe all listeners that may have hooked into the SB for security breach
   * events. This is required to make sure that we dont leak memory
   */
  ...
  Firefox.prototype.restoreKeyboardLayoutSettings = function() {
    ...
    if (Mozilla.getPreference('windows.previouslanguagehotkeyvalue') == '') {
      ...
      this.writeRegistryValue(Util.SecureBrowser.HKEY_CURRENT_USER,
          "Keyboard Layout\\toggle", "Language Hotkey", "string", Mozilla
    ...
    if (Mozilla.getPreference('windows.previouslayouthotkeyvalue') == '') {
      ...
      this.writeRegistryValue(Util.SecureBrowser.HKEY_CURRENT_USER,
          "Keyboard Layout\\toggle", "Layout Hotkey", "string", Mozilla
              .getPreference('windows.previouslayouthotkeyvalue'));
    }
    // remove the value from the local storage
