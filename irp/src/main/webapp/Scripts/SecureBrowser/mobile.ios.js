...
TDS.SecureBrowser.Mobile.iOS = function () {
    ...
    this._defaultBackgroundThresholdASAM = 20;  // we allow up to twenty seconds for the browser app to be backgrounded if ASAM is on
...
TDS.SecureBrowser.Mobile.iOS.prototype.initialize = function () {
    ...
    // a function to check if Automatic Assessment Configurator is enabled for a given iOS version (yes for iOS 9.3.2 or later)
    this.isAACEnabled = function () {
        ...
        // the version retrieved from iOS brower contains build id which should be ignored; an example iOS version is '8.3 12F69'
        ...
        if (versions[0] > 9 || (versions[0] == 9 && versions[1] == 3 && versions[2] >= 2)) {
            return true;
        }
        return false;
    };
    ...
    this.getBackgroundThreshold = function () {
        if (typeof (backgroundThreshold) == 'undefined') {
            backgroundThreshold = (typeof TDS != 'undefined') ? TDS.getAppSetting('sb.iosBackgroundThreshold', this._defaultBackgroundThreshold) : this._defaultBackgroundThreshold;
        }
    ...
    this.getBackgroundThresholdASAM = function () {
        if (typeof (backgroundThresholdASAM) == 'undefined') {
            backgroundThresholdASAM = (typeof TDS != 'undefined') ? TDS.getAppSetting('sb.iosASAMBackgroundThreshold', this._defaultBackgroundThresholdASAM) : this._defaultBackgroundThresholdASAM;
        }
    ...
    this.setLockDown = function (lockdown) {
        ...
        } else if (this.checkAutonomousGuidedAccess() && guidedAccessMode == 'disabled') {
    ...
    secBrowser.listen(secBrowser.EVENT_RETURN_FROM_BACKGROUND, document, function () {
        ...
        if (isLockedDown) {
            ...
            if (startTimeBackground != null) {
                ...
                var threshold = (ASAMMode == 'enabled') ? self.getBackgroundThresholdASAM() : self.getBackgroundThreshold();
