...
(function(SB) {
	...
	/*
	 * The base class for all secure browsers. Any new secure browser must
	 * extend from this and override the methods to provide custom behavior if
	 * required
	 */
	function Base() {
					...
					.each(
							...
							function(currentProcess) {
								...
								if (currentProcess.indexOf("|") > -1) {
									var processInfoArray = currentProcess
											.split("|");
									processName = processInfoArray[0];
									parentProcessName = processInfoArray[1];
								}
								var matchingApp = Util.Array
										.find(
												TDS.Config.forbiddenApps,
												function(forbiddenApp) {
													...
													if (Util.String
														...
														// forbidden
														if (Util.String
																.isNullOrEmpty(parentProcessName)
																|| Util.String
																		.isNullOrEmpty(forbiddenApp.exemptions)) {
															return true;
														} else {
															// check if the
															// parent process
															...
															// any exemption
															// processes
															var exemptionList = forbiddenApp.exemptions
																	.split(",");
															var exemptionMatchFound = Util.Array
																	.find(
																			exemptionList,
																			function(
																					exemption) {
