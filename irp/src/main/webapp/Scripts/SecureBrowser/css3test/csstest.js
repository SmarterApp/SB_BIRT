...
var Test = function(tests, spec, title) {
  ...
  var h1 = $u.element.create({
    tag : 'h1',
    contents : [ this.title, $u.element.create({
      tag : 'a',
      properties : {
...
Test.prototype = {
  group : function(what, testCallback) {
    ...
    for ( var feature in theseTests) {
      ...
      var dl = document.createElement('dl'), dt = $u.element.create({
        tag : 'dt',
        prop : {
          textContent : feature,
          tabIndex : '0'
      ...
      for (var i = 0, test; test = tests[i++];) {
        var results = testCallback(test, feature, theseTests), success, note;
      ...
      // Add to browserscope
      _bTestResults[this.id + ' / ' + feature.replace(/[,=]/g, '')] = Math
          .round(100 * passed / tests.length);
    }
  }
}
...
Test.groups = {
  'values' : function(test, label, tests) {
    var properties = tests.properties, failed = [];
...
function passclass(info) {
  ...
  var classes = [ 'epic-fail', 'fail', 'very-buggy', 'buggy', 'slightly-buggy',
      'almost-pass', 'pass' ];
...
onload = function() {
  var timeBefore = +new Date, duration = 0;
...
function populateCSS3ScoreHTML(mainScore) {
  ...
  css3ScoreHTML = '<span id="extScore"> [Score <strong>' + mainScore.percent()
      + '%</strong>]</span>';
