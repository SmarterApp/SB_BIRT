...
var tim = (function() {
  var starts = "\\{\\{", ends = "\\}\\}", path = "[a-z0-9_][\\.a-z0-9_]*", // e.g.
  // config.person.name
  pattern = new RegExp(starts + "(" + path + ")" + ends, "gim"), undef;
  ...
  return function(template, data, notFound) {
    ...
    return template.replace(pattern, function(tag, ref) {
      var path = ref.split("."), len = path.length, lookup = data, i = 0;
...
function escapeSlashes(string) {
  return string.replace(/\\/g, '\\\\').replace(/\u0008/g, '\\b').replace(/\t/g,
...
Calculate.prototype = {
  ...
  iterate : function(data, prefix) {
    for (var i = 0; i < data.length; i++) {
      if (data[i].key) {
        ...
        if (prefix == '') {
          this.points.push(data[i].id + '=' + (this.score - score) + '/'
  ...
  calculate : function(prefix, data) {
    var result = true;
    var value = typeof data.value == 'object' ? data.value : {
      maximum : data.value
    };
    ...
    if (result & YES) {
      ...
      if (valid) {
        if (result & PREFIX && typeof value.award == 'object'
            && typeof value.award.PREFIX != 'undefined') {
          this.score += value.award.PREFIX;
        } else if (result & BUGGY && typeof value.award == 'object'
            && typeof value.award.BUGGY != 'undefined') {
          this.score += value.award.BUGGY;
        } else if (result & OLD && typeof value.award == 'object'
            && typeof value.award.OLD != 'undefined') {
  ...
  getResult : function(key) {
    if (match = (new RegExp(key + '=(-?[0-9]+)'))
        .exec(this.parameters.test.results)) {
...
Share.prototype = {
  ...
  create : function() {
    ...
    this.popup.innerHTML += "<div id='share'><div>"
        +
        ...
        + " scored "
        + this.options.score
        + " points. How well does your browser support HTML5?' "
        + "data-lang='en' "
        + "data-count='vertical'"
        + ">Tweet</a>"
        + "</div>"
        ...
        "<div id='google'>" + "<div class='g-plusone' "
        + "data-href='https://html5test.com' " + "data-size='tall'" + "></div>"
        + "</div>" +
...
Save.prototype = {
  ...
  create : function() {
    ...
    this.popup.innerHTML += "<div id='save'>"
        ...
        + encodeURIComponent(document.location.protocol + "//html5te.st/"
            + this.options.id) + "&choe=UTF-8' width='200' height='200'>"
        + "</p>" + "<p>The unique id for this test is:<br><code>"
        + this.options.id + "</code></p>" + "</div>";
  ...
  open : function(e) {
    ...
    while (el != this.parent && ignore == false) {
      if (el.className.indexOf('popupPanel') != -1)
        ignore = true;
      el = el.parentNode;
    }
...
Feedback.prototype = {
  ...
  create : function() {
    ...
    this.popup.innerHTML += "<div id='feedback'>"
        + "<h3>What is wrong with this identification?</h3>"
        + "<textarea id='correction'>" + this.options.suggestion
        + "</textarea>"
...
ResultsTable.prototype = {
          ...
          if (typeof test != 'undefined') {
            ...
            if (this.options.grading) {
              ...
              if (points == maximum)
                ...
                content += "<span class='" + grade + "'>" + points + "/"
                    + maximum + "</span>";
            } else {
        ...
        if (typeof tests[i].items != 'undefined') {
          ...
          if (match = (new RegExp(key + '=(-?[0-9]+)')).exec(data.results)) {
            ...
            if (result & YES) {
              switch (true) {
              case !!(result & BUGGY):
                cell.innerHTML = '<div>Buggy <span class="buggy"></span></div>';
                break;
              case !!(result & OLD):
                cell.innerHTML = '<div>Partial <span class="partial">○</span></div>';
                count[1]++;
                break;
              case !!(result & PREFIX):
                cell.innerHTML = '<div>Prefixed <span class="check">✔</span></div>';
                count[1]++;
                break;
              default:
                cell.innerHTML = '<div>Yes <span class="check">✔</span></div>';
              ...
              switch (true) {
              case !!(result & UNKNOWN):
                cell.innerHTML = '<div>Unknown <span class="partial">?</span></div>';
                break;
              case !!(result & BLOCKED):
                cell.innerHTML = '<div>Not functional <span class="buggy">!</span></div>';
                break;
              case !!(result & DISABLED):
                cell.innerHTML = '<div>Disabled <span class="ballot">✘</span></div>';
                break;
              default:
                cell.innerHTML = '<div>No <span class="ballot">✘</span></div>';
                break;
  ...
  createCategories : function(parent, tests) {
    ...
    for (var i = 0; i < tests.length; i++) {
      ...
      if (tests[i].column == 'left')
        container = left.firstChild;
      if (tests[i].column == 'right')
        container = right.firstChild;
      ...
      if (typeof tests[i] == 'string') {
        ...
        if (typeof tests[i].items != 'undefined') {
          ...
          var children = this.createItems(parent, level + 1, tests[i].items, {
            id : key,
            status : typeof tests[i].status != 'undefined' ? tests[i].status
                : data.status,
            urls : urls
          });
...
FeaturePopup.prototype = {
  initialize : function(parent, data) {
    ...
    content += "<div class='column left status " + data.status + "'><span>"
        + data.status + "</span></div>";
    content += "<div class='column middle" + (maximum ? '' : ' none')
        + "'><em>" + (maximum || '✘') + "</em> <span>"
        + (maximum != 1 ? 'Points' : 'Point') + "</span></div>";
    content += "<div class='column right'><a href='/compare/feature/" + data.id
        + ".html' class='compare'><span>Compare</span></a></div>";
    content += "</div>";
    content += "<div class='links'>";

    for (var i = 0; i < data.urls.length; i++) {
      if (data.urls[i][0] == 'w3c')
        content += "<a href='" + data.urls[i][1]
            + "' class='w3c'>Go to the specification at W3C.org</a>";
      if (data.urls[i][0] == 'whatwg')
        content += "<a href='" + data.urls[i][1]
            + "' class='whatwg'>Go to the specification at Whatwg.org</a>";
      if (data.urls[i][0] == 'khronos')
        content += "<a href='" + data.urls[i][1]
            + "' class='khronos'>Go to the specification at Khronos.org</a>";
      if (data.urls[i][0] == 'ietf')
        content += "<a href='" + data.urls[i][1]
            + "' class='ietf'>Go to the specification at IETF.org</a>";
      if (data.urls[i][0] == 'webm')
        content += "<a href='" + data.urls[i][1]
            + "' class='webm'>Go to the specification at WebMproject.org</a>";
      if (data.urls[i][0] == 'xiph')
        content += "<a href='" + data.urls[i][1]
            + "' class='xiph'>Go to the specification at Xiph.org</a>";
      if (data.urls[i][0] == 'ecma')
        content += "<a href='" + data.urls[i][1]
            + "' class='ecma'>Go to the specification at ECMA</a>";
      if (data.urls[i][0] == 'ricg')
        content += "<a href='" + data.urls[i][1]
            + "' class='ricg'>Go to Responsive Images Community Group</a>";
      if (data.urls[i][0] == 'wp')
        content += "<a href='https://docs.webplatform.org/wiki"
            + data.urls[i][1]
            + "' class='wp'>Documentation at WebPlatform.org</a>";
      if (data.urls[i][0] == 'mdn')
        content += "<a href='https://developer.mozilla.org/en-US/docs"
            + data.urls[i][1]
            + "' class='mdn'>Documentation at Mozilla Developer Network</a>";
    }
